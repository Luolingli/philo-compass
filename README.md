...
测测你的思想属于哪位哲学家。思想卷(标准 60 题 / 深度 120 题)× 15 道环境题，像 MBTI 一样看清你的思想成分。
...
- `src/data/`：`dimensions.ts`（12 维坐标空间）、`schools.ts`（52 流派向量）、
  `philosophers.ts`（100 位思想家：流派 + 微调 + 代表作 + 金句）、`questions.ts`（标准 60 + 深度扩展 60 + 环境 15 题；
  每题标注"帮助区分的流派"，52 流派全覆盖、无死角）
